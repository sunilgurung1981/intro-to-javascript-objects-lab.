...
const threeMore = pokemon.sort(() => Math.random());
game.party.push(...threeMore.slice(0, 3));
console.log(game.party);
...
for (let i = 0; i < party.length; i++) {
  if (party[i].starter) {
    const evolvedForm = evolutionMapping[party[i].number];
    if (evolvedForm) {
      party.splice(i, 1, evolvedForm);
    }
  }
}
console.log(party);
